...
function M.reset_rails_db()
  require("tmux-awesome-manager").execute_command({
    cmd = "cd ~/Projetos/shun; yarn db:reset; cd ~/Projetos/api; bin/rails db:environment:set RAILS_ENV=development; rails db:drop db:create db:migrate;rails db:seed",
    name = "db:reset",
    open_as = "pane",
...
function M.find_in_folder(folder, title)
  return function()
    if vim.fn.isdirectory(folder) == 1 then
      vim.cmd(
        "lua require'telescope.builtin'.find_files({ cwd = '" .. folder .. "', prompt_title = '" .. title .. "' })"
      )
    else
